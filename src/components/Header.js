...
const menu = [
  ...
  { label: 'Events', href: '/events', isButton: true },
          ...
          <nav className={`nav ${mobileOpen ? 'open' : ''}`}>
            {menu.map((item) =>
              item.children ? (
                ...
                <Link key={item.label} href={item.href} className="nav-link nav-link-btn" onClick={() => setMobileOpen(false)}>
